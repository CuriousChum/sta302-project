...
# Analysis on right-handed pitcher
## Task 0: Setup
...
create_response_hist <- function(responses){
...
create_response_hist(responses)
```
...
create_residual_plot <- function(data, x_var, x_lab, title, e_hat) {
...
create_resid_vs_pred_plot <- function(dataframe, filename, e_hat){
  # Residuals vs Predictors
  p_resid_rs <- create_residual_plot(dataframe, release_speed, "Release Speed", "Residuals vs Release Speed", e_hat)
  p_resid_rsr <- create_residual_plot(dataframe, release_spin_rate, "Release Spin Rate", "Residuals vs Release Spin Rate", e_hat)
  p_resid_re <- create_residual_plot(dataframe, release_extension, "Release Extension", "Residuals vs Release Extension", e_hat)
  p_resid_rpz <- create_residual_plot(dataframe, release_pos_z, "Release Pos Z", "Residuals vs Release Pos Z", e_hat)
  p_resid_rpx <- create_residual_plot(dataframe, release_pos_x, "Release Pos X", "Residuals vs Release Pos X", e_hat)
  p_resid_pz <- create_residual_plot(dataframe, plate_z, "Plate Z", "Residuals vs Plate Z", e_hat)
  p_resid_px <- create_residual_plot(dataframe, plate_x, "Plate X", "Residuals vs Plate X", e_hat)
  p_resid_az <- create_residual_plot(dataframe, az, "Az", "Residuals vs Az", e_hat)
  p_resid_ax <- create_residual_plot(dataframe, ax, "Ax", "Residuals vs Ax", e_hat)
  p_resid_pfx_z <- create_residual_plot(dataframe, pfx_z, "Pfx Z", "Residuals vs Pfx Z", e_hat)
  p_resid_pfx_x <- create_residual_plot(dataframe, pfx_x, "Pfx X", "Residuals vs Pfx X", e_hat)
  
  # Special case for boxplot
  p_resid_s <- ggplot(data = dataframe, aes(x = stand, y = e_hat)) +
  ...
  # Saved as filename in the working directory
  ggsave(filename, 
        combined_plot, 
        width = 15, 
        height = 12, 
...
create_resid_vs_pred_plot(RHP_FB0, "residual_plots_RHP.jpg", e_hat)

```

```{r}
# Residuals vs fitted values
create_resid_fitted_plot <- function(y_hat, e_hat){
  p_resid_fitted <- ggplot(data = data.frame(fitted = y_hat, residuals = e_hat), aes(x = fitted, y = residuals)) +
    geom_point(alpha = 0.6, color = "#2c3e50", size = 1.5) +
    geom_smooth(method = "loess", se = TRUE, color = "#e74c3c", fill = "#ffcdd2", alpha = 0.2) +
...
create_pairs_plot <- function(dataframe){
  predictors_df <- dataframe[, c("release_speed", "release_spin_rate", "release_extension", 
  ...
  #       height = 20, 
  #       dpi = 300)
  rm(p_pairs, predictors_df)
}

create_resid_fitted_plot(y_hat, e_hat)
create_qq_plot(e_hat)
# create_pairs_plot(RHP_FB0)
```

## Variable transformation
```{r}
summary(powerTransform(cbind(RHP_FB0[,15])))
```

```{r}
# Transform our response variable and do another filter for release pos z (due to special case of a person's (Adam Cimber) pitch specialty of throwing ball as low as possible)
RHP_FB0 <- RHP_FB0 |> 
        mutate(whiff = whiff^(1/2)) |> 
...
```{r}
# Response variable distribution of transformed data
create_response_hist(responses)
# Residuals vs fitted values for model with transformed data
create_resid_fitted_plot(y_hat, e_hat)
...
We immediately see that the *Adjusted R-squared* is 0.5145, and the *p-value* is \< 2.2e-16, which is promising.
...
We also observe no bias in the residuals, and no distinct skew/patterns after the transformation.
...
```{r}
# Check for any assumptions violated
create_resid_fitted_plot(y_hat_temp, e_hat_temp)
create_qq_plot(e_hat_temp)
```

Slight deviation on the left side of the QQ-plot, but is not significant enough. Slight fanning pattern can be seen, but is also not significant enough to pose issue. 
```{r}
# Perform ANOVA test
...
```{r}
y_hat <- resid(Final_lm)
e_hat <- fitted(Final_lm)
# Residual vs Fitted plot
create_resid_fitted_plot(y_hat, e_hat)
# Create QQ plot
create_qq_plot(e_hat)
...
# Analysis on left-handed pitcher
## Data Cleaning

```{r}
# Filter data for left-handed pitchers (LHP) and four-seam fastballs (FF)
# Filter for all swings and calculate whiff rates
LHP_FB <- savant_data |>
  filter(p_throws == "L", pitch_type == "FF",
         description %in% c("hit_into_play", "swinging_strike", "foul", "foul_tip")) |>
  mutate(whiff = if_else(description == "swinging_strike", 1, 0))
```
...
create_resid_vs_pred_plot(LHP_FB0, "residual_plots_LHP.jpg", e_hat)
```
...
# Normal Q-Q plot
create_qq_plot(e_hat)
# create_pairs_plot(LHP_FB0)
```

## Variable Transformation
```{r}
summary(powerTransform(cbind(LHP_FB0[,15])))
```
...
```{r}
# Create predictions for whiff rate
Predict_LHP_FB0 <- LHP_FB0 |>
  ungroup() |>
  mutate(est_whiff = predict(fb_model_updated, newdata = LHP_FB0))

# Group predictions by player and year
Final <- Predict_LHP_FB0 |>
  group_by(player_name, game_year) |>
  summarise(new_count = sum(count),
            whiff = sum(whiff * count) / sum(count),
            est_whiff = sum(est_whiff * count) / sum(count))
